...
onMounted(async () => {
  heroContent.value = await apiCalls.retrievePage('nwroster')
  ...
  await apiCalls.retrievePeopleByTag('nw-roster', firstTagId).then((data) => {
...
function retrievePeopleByTag(tagName) {
  ...
  apiCalls.retrievePeopleByTag('nw-roster', tagId).then((data) => {
    postContent.value = data
  })
}
</script>
