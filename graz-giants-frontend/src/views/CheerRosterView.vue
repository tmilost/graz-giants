...
onMounted(async () => {
  heroContent.value = await apiCalls.retrievePage('cheerroster')
  ...
  await apiCalls.retrievePeopleByTag('cheerroster', firstTagId).then((data) => {
...
function retrievePeopleByTag(tagName) {
  ...
  apiCalls.retrievePeopleByTag('cheerroster', tagId).then((data) => {
    postContent.value = data
  })
}
</script>
